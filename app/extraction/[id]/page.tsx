...
export default function ExtractionPage() {
  ...
  const id = typeof idValue === "string" ? idValue : "proj-0";
  ...
  useEffect(() => {
    ...
    if (saved) {
      try {
        ...
        targetProj = list.find((p: any) => p.id === id);
      } catch (e) {
        console.error(e);
      }
    }
    ...
    if (!targetProj) {
      ...
      targetProj = {
        id: id,
        name: id === "api-service" ? "api-service" : id === "web-frontend" ? "web-frontend" : "Momentum Dashboard",
        color: "#06B6D4",
  ...
  // Auto scroll node list when nodes change
  ...
  const handleUserSubmit = (overrideText?: string, explicitType?: string) => {
    ...
    setTimeout(() => {
      ...
      // STEP 7: Quality Gate Rejection (Simulating a vague response check on Q7)
        ...
        // Trigger simulated vague response checks
        setMessages(prev => [
          ...prev,
          {
        ...
        // decrement index by 1 so the next reply properly completes step 7
        setStepIndex(6);
      }
      ...
      // STEP 9: Contradictory Input Event Trigger
        ...
        // decrement index by 1 to make resolution step complete step 9
        setStepIndex(8);
      }
  ...
  if (isLoading || !project) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[100dvh] bg-[#090a0f] text-white">
        ...
        <span className="text-sm font-mono tracking-widest uppercase opacity-75">Loading workspace context...</span>
      </div>
    );
  }

  return (
    <main className="w-full h-[100dvh] min-h-[100dvh] bg-[#090a0f] text-white flex flex-col overflow-hidden relative">
      {/* GLOW DECORATIONS (Microscale ambient shadows only, no flashy overlays) */}
      <div className="absolute top-0 left-1/4 w-[500px] h-[500px] bg-cyan-950/20 rounded-full blur-[150px] pointer-events-none z-0" />
      <div className="absolute bottom-0 right-1/4 w-[400px] h-[400px] bg-blue-950/20 rounded-full blur-[130px] pointer-events-none z-0" />
      ...
      <header className="w-full h-14 bg-[#0d0e14]/90 border-b border-white/5 backdrop-blur-md flex items-center justify-between px-6 z-10 shrink-0">
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2 cursor-pointer" onClick={handleConfirmAndExit}>
            <span className="w-6 h-6 rounded-lg bg-gradient-to-br from-cyan-400 to-blue-600 flex items-center justify-center font-bold text-[11px] text-black">
              M
            </span>
            <span className="text-sm font-bold tracking-tight font-sans">Momentum</span>
          </div>
        ...
        <div className="flex items-center gap-4">
          ...
          <button
            ...
            title="Save your progress & return to dashboard"
          >
      ...
      {/* TWO PANEL WORKSPACE */}
      <div className="w-full flex-1 flex overflow-hidden z-10">

        {/* LEFT PANEL — CONVERSATION EXPERIENCE (60%) */}
        <section className="w-[60%] flex flex-col border-r border-white/5 bg-[#0a0b10] relative overflow-hidden">
          
          {/* Panel Header */}
          <div className="px-6 py-4 border-b border-white/5 bg-[#0d0e14]/40 flex items-center justify-between">
            <div>
              ...
              <p className="text-[11px] text-white/50 font-mono mt-0.5">
                Momentum AI · Extracting {sections[activeStep]?.name || "Organization Profile"}
              </p>
            </div>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex flex-col gap-1.5">
                      {/* Message Bubble Container with custom border styling for triggers */}
                      <div
                      ...
                      >
                        {/* Interactive dynamic trigger labels inside bubble */}
                        {m.qualityWarning && (
                        ...
                        {/* Inline custom templates with interactive inputs inside message bubble */}
                        {isAgent && m.type === "template" && stepIndex === 2 && (
                          <div className="mt-4 p-3 rounded-xl bg-black/40 border border-white/5 flex flex-col gap-3">
                            ...
                            <div className="text-sm leading-8 text-white/90">
                              We help{" "}
                              <input
                                type="text"
                                spellCheck={false}
                                ...
                                className="inline-block bg-white/5 border-b border-white/30 text-cyan-400 rounded px-1.5 py-0.5 max-w-[150px] outline-none text-center focus:border-cyan-400"
                              />{" "}
                              do{" "}
                              <input
                                type="text"
                                ...
                                className="inline-block bg-white/5 border-b border-white/30 text-cyan-400 rounded px-1.5 py-0.5 max-w-[200px] outline-none text-center focus:border-cyan-400"
                              />{" "}
                              so they can{" "}
                              <input
                                type="text"
                        ...
                        {/* Interactive drag-ranking structure inside bubble */}
                        ...
                        {/* Interactive Scale slider inside message bubble */}
                        ...
                        {/* Real-time structured Conflict side-by-side card within message */}
                          ...
                          <div className="mt-4 grid grid-cols-2 gap-3">
                            <div className="p-3 rounded-xl bg-rose-500/5 border border-rose-500/25 flex flex-col justify-between">
                              <div>
                                ...
                                <p className="text-xs text-white/80 leading-relaxed">{m.conflictDetails?.original}</p>
                              </div>
                              <button
                                onClick={() => handleUserSubmit("original")}
                                className="mt-4 py-1.5 bg-rose-500/10 hover:bg-rose-500/20 text-rose-400 border border-rose-500/20 rounded-lg font-mono text-[10px] uppercase font-bold cursor-pointer"
                              >
                                Keep Original
                              </button>
                            </div>
                            ...
                            <div className="p-3 rounded-xl bg-cyan-500/5 border border-cyan-500/25 flex flex-col justify-between">
                              <div>
                                ...
                                <p className="text-xs text-white/80 leading-relaxed">{m.conflictDetails?.conflict}</p>
                              </div>
                              <button
                                onClick={() => handleUserSubmit("conflict")}
                                className="mt-4 py-1.5 bg-cyan-500/15 hover:bg-cyan-500/25 text-cyan-400 border border-cyan-500/20 rounded-lg font-mono text-[10px] uppercase font-bold cursor-pointer"
                              >
                                Resolve to B
                              </button>
                            </div>
                        ...
                        {/* Premium confirmation summary node within chat */}
                        {isAgent && m.type === "confirm" && stepIndex === 10 && (
                          <div className="mt-4 p-4 rounded-2xl bg-black/40 border border-cyan-500/15">
                            ...
                            <div className="space-y-2 mb-4">
                              <div className="flex border-b border-white/5 pb-1 justify-between text-xs font-mono">
                        ...
                        {/* Final generate documentation success card */}
                        {m.text.includes("concludes our structured knowledge extraction session") && (
                          <div className="mt-4">
                            <button
                              onClick={() => {
                      ...
                      {/* Display custom Selectable Pill Chips immediately below bubble (Format 2) */}
                      ...
                      {/* Display Question with Examples immediately inside bubble (Format 3) */}
                      {isAgent && m.type === "examples" && stepIndex === 1 && (
                        <div className="mt-2.5 p-3 rounded-lg bg-black/35 border border-white/5 flex flex-col gap-2">
                          ...
                          {m.examples?.map((ex, i) => (
                            <div key={i} className="text-xs text-white/50 italic leading-relaxed pl-2 border-l border-white/15">
                              {ex}
                            </div>
                          ))}
                        </div>
                      ...
                      {/* Display Complex Structured Question Cluster (Format 4) */}
          ...
          {/* INPUT PORTAL FOOTER (Fixed at base) */}
          <div className="p-4 border-t border-white/5 bg-[#0d0e14]/70 z-10">
            <div className="flex items-center gap-3">
              <div className="flex-1 relative flex items-center bg-[#11121a] rounded-xl border border-white/10 focus-within:border-cyan-500/50 transition-colors">
                ...
                <div className="absolute right-3 flex items-center gap-2">
                  ...
                  <button
                    onClick={() => textInput.trim() && handleUserSubmit()}
            ...
            {/* Input helpers and skipping logic */}
            <div className="flex items-center justify-between text-[11px] font-mono text-white/40 mt-3 select-none">
              ...
              <div className="flex items-center gap-3">
                <span className="opacity-75">Progress auto-saved · Session 1</span>
                <span>|</span>
                <button
        ...
        <section className="w-[40%] h-full flex flex-col bg-[#0b0c11] relative overflow-hidden">
          
          {/* Header Panel */}
          <div className="px-6 py-4 border-b border-white/5 bg-[#0d0e14]/30 flex items-center justify-between shrink-0">
            <div>
              <h2 className="text-sm font-bold font-sans text-white tracking-widest uppercase flex items-center gap-2">
                Knowledge Capture
                <span className="w-1.5 h-1.5 rounded-full bg-cyan-400 animate-ping" />
              </h2>
              <p className="text-[11px] text-white/50 font-mono mt-0.5">
                Updating live as you outline specifications
              </p>
            </div>
          </div>

          {/* Collapsible Section Tree / Outline Node */}
              ...
              {sections.map((sec, idx) => {
                ...
                return (
                  ...
                  >
                    ...
                    {/* Explorable Nested mapping nodes inside directory */}
                    {isOpen && (
                      <div className="px-4 pb-3 pt-1 border-t border-white/5 font-mono text-[11px] text-white/60 space-y-2 select-none">
                        {sec.id === "org" && (
                          <>
                            ...
                            <div className="flex justify-between items-center opacity-85">
                              ...
                              <span className={stepIndex > 1 ? "text-emerald-400 font-bold" : "text-amber-400 animate-pulse"}>
          ...
          {/* LIVE NODE FEED LOG (Center Area scrollable) */}
          ...
          {/* COMPLETENESS DIAL FOOTER PANELS */}
            ...
            <div className="flex items-center gap-5">
              ...
              <div className="flex-1 flex flex-col justify-center">
                ...
                <p className="text-[11px] font-mono text-white/40 mt-1">
                  {capturedNodes.length} active database schema specifications resolved to workspace index cache.
                </p>
